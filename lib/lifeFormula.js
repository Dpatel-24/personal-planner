...
// The 9 scored inputs' display metadata, grouped into the three zones the
// entry-form spec calls for (Numerator/Coherence, Denominator/Resistance,
// Multiplier/Execution). Pulled out here (rather than left local to
// pages/life-formula.js, where it originally lived) so the Log view
// (pages/life-formula-log.js) can render the exact same grouping/labels for
// editing an existing entry instead of a second, driftable copy — this file
// already owns the 9 keys' names and meaning via calculateLifeFormula below,
// so it's the natural single source of truth for them too. Color tokens
// referenced directly (not passed in) since both consumers use the same
// palette regardless of caller.
import { color } from './tokens';

export const LIFE_FORMULA_ZONES = [
  {
...
export const LIFE_FORMULA_KEYS = LIFE_FORMULA_ZONES.flatMap((z) => z.metrics.map(([key]) => key));

// State-name -> display color, shared by the dashboard's own charts/badges
// and the Log's state pill (pages/life-formula-log.js) — one mapping for
// the three classifyState() bands, not a copy per consumer.
...
// Exported so other aggregations (e.g. lib/lifeFormulaStats.js's monthly
// rollup, which classifies a MONTH's averaged score, not a raw formula
// input) reuse these exact thresholds instead of a second, driftable copy.
export function classifyState(score) {
