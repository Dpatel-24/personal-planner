...
// Every ISO week label from startLabel through endLabel (inclusive),
// oldest-first — the Life Formula Log's own week axis (pages/
// life-formula-log.js), so a week with no entry still gets a row instead of
// just vanishing from the list. Walks real Thursday-anchor dates 7 days
// apart (isoWeekToDate/isoWeekLabel, both already ISO-year-boundary-correct)
// rather than incrementing a week NUMBER, so a 52-vs-53-week year never
// needs special-casing here. endLabel defaults to THIS INSTANT's week
// (isoWeekLabel(), evaluated at call time, not cached) — that's what keeps
// the Log's own upper bound correct as real time and new logged weeks move
// forward, with no separate "today" plumbing needed from the caller.
export function weekLabelsFrom(startLabel, endLabel = isoWeekLabel()) {
  const labels = [];
  let cursorMs = isoWeekToDate(startLabel).getTime();
  const endMs = isoWeekToDate(endLabel).getTime();
  while (cursorMs <= endMs) {
    labels.push(isoWeekLabel(new Date(cursorMs)));
    cursorMs += 7 * 86400000;
  }
  return labels;
}
