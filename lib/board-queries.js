...
// checklist_items(id, is_done, text, position) embedded for the card-face
// "X/Y" progress count AND (2026-09) the actual item breakdown rendered
// under the card on the board — text/position added alongside the
// pre-existing id/is_done specifically so WeekBoardCard.js can show every
// item's own label without a per-card getChecklistItems() round trip, per
// the ask ("I should not have to click on the card to see the breakdown").
// Independent of tags, added the same way as data.js. time_entries
// (started_at, ended_at) same embed pattern, for the card-face total-
...
const INSTANCE_SELECT =
  '*, tagRow:tags(id, name, color), checklist_items(id, is_done, text, position), time_entries(started_at, ended_at), template:task_templates(id, title, description, recurrence_rule, start_date, end_date, active, defaultTagRow:tags(id, name, color))';
...
function resolveInstance(row) {
  ...
  // Sorted by position here (not via a second .order() query param) since
  // it's already the whole embedded array in memory — WeekBoardCard.js's
  // item breakdown renders in this order, matching the same order the
  // checklist's own drag-reorder (ChecklistSection.js) puts them in.
  const checklist = [...(row.checklist_items ?? [])].sort((a, b) => a.position - b.position);
  ...
  return {
    ...
    checklist_items: checklist, // overrides the ...row spread above with the position-sorted copy
...
// One day column. isToday=false: just that date, excluding incomplete todos
// from past days (those are carried over to today's column only).
// isToday=true: that date (any status) PLUS the rollover set — earlier dates
// still 'todo' (CLAUDE.md decision: rollover is a computed query, not a data
// mutation). Rolled-over rows get a computed is_overdue: true; same-day rows
// get false.
export async function getColumnInstances(date, isToday, client = supabase) {
  const dateStr = toDateStr(date);
  const todayStr = toDateStr(new Date());

  let query = client.from('task_instances').select(INSTANCE_SELECT).eq('scheduled_date', dateStr);

  // For non-today columns in the past: exclude status='todo' (those are rolled over)
  if (!isToday && dateStr < todayStr) {
    query = query.neq('status', 'todo');
  }

  const { data: sameDay, error: sameDayErr } = await query.order('position', {
    ascending: true,
    nullsFirst: true,
  });
  ...
  if (!isToday) return sortWithPins(sameDayRows);
  ...
  return sortWithPins([...rolloverRows, ...sameDayRows]);
}
