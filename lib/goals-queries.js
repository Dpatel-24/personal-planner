...
const GOALS_SELECT = 'id, parent_id, title, category, is_complete, created_at, updated_at';
...
// One goal with effective_category resolved. Goes through listGoals() rather
// than a second, separate walking strategy — one derivation function, one
// place it can go wrong, same reasoning as resolveInstance() being the ONE
// place title/description inheritance is computed. Fine at this app's scale
// (single user, a personal goal list); a large goal count would be the
// signal to switch this to a recursive SQL query instead, not a reason to
// add a second JS-side walking strategy now.
export async function getGoal(id, client = supabase) {
  const all = await listGoals(client);
...
// Percentage (0-100) of complete leaf descendants under goalId, computed by
// the get_goal_progress(goal_id) Postgres function (WITH RECURSIVE walk to
// leaves — see its migration). Called via supabase.rpc(), same pattern
// lib/timer-queries.js's startTimer() already uses for start_timer() — the
// one other place this app calls a DB function instead of a plain table
// query, both times because the logic (atomic stop-then-insert; recursive
// tree aggregation) doesn't belong client-side.
export async function getGoalProgress(goalId, client = supabase) {
  const { data, error } = await client.rpc('get_goal_progress', { goal_id: goalId });
  if (error) throw error;
  return Number(data);
}
