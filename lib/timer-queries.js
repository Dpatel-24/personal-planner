...
// Stop the active timer ONLY if it belongs to instanceId — a no-op if that
// instance isn't the one currently running (including if nothing is
// running at all). Since at most one row can ever have ended_at IS NULL
// (see the module header), scoping the WHERE by instance_id already implies
// "only if this is the active one" without a separate read-then-write.
// Used when a task is marked done (CLAUDE.md: timer stops on completion).
export async function stopTimerForInstance(instanceId, client = supabase) {
  const { error } = await client
    .from('time_entries')
    .update({ ended_at: new Date().toISOString() })
