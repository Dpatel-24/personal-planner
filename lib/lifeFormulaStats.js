...
// Corrects an EXISTING week's answers (Log view, pages/life-formula-log.js)
// — same score/state-computed-and-stored-at-write-time shape as
// createLifeFormulaEntry above, just an update instead of an insert, and
// never touches week_label (correcting an existing week's rating is not the
// same operation as logging a different week — no route for that here).
export async function updateLifeFormulaEntry(id, inputs, client = supabase) {
  const { score, state } = calculateLifeFormula(inputs);
  const { data, error } = await client
    .from('life_formula_entries')
    .update({ ...inputs, score, state })
    .eq('id', id)
    .select()
    .single();
  if (error) throw error;
