...
export async function generateInstances(
  ...
  if (toInsert.length) {
    const rows = toInsert.map((date) => ({
      ...
      status: 'todo',
      is_override: false,
      // Copy the template's CURRENT default tag onto the new row only —
      // frozen at generation time, same disposable-until-touched pattern as
      // the checklist copy below. Never live-joined at read time, so a later
      // change to default_tag_id can't retroactively affect this instance.
      tag_id: template.default_tag_id ?? null,
      // Same copy-at-generation-time pattern for pinned_position (Morning
      // Chain/Evening Winddown) — a normal template's pinned_position is
      // null, so this is a no-op for every other recurring task. Render/
      // sort logic (components that list a day's tasks) reads this column
      // directly off the instance, never the template.
      pinned_position: template.pinned_position ?? null,
    }));
    const { data: insertedRows, error: insErr } = await client
      .from('task_instances')
      .insert(rows)
      .select('id');
    if (insErr) throw insErr;

    // Copy this template's checklist onto each brand-new instance only —
    // never onto rows that already existed. Retroactively pushing a
    // checklist onto already-generated instances is a separate, deliberate
    // step lib/data.js's updateTemplateAll performs on its own (EditModal's
    // "All occurrences" scope) — this function's job stays narrow: give a
    // newly-materialized occurrence the template's CURRENT checklist,
    // nothing more.
    const { data: templateChecklist, error: tclErr } = await client
      .from('checklist_templates')
      .select('text, position')
      .eq('template_id', template.id)
      .order('position', { ascending: true });
    if (tclErr) throw tclErr;

    if (templateChecklist?.length && insertedRows?.length) {
      const checklistRows = insertedRows.flatMap((inst) =>
      ...
      const { error: clInsErr } = await client.from('checklist_items').insert(checklistRows);
      if (clInsErr) throw clInsErr;
    }
  }
