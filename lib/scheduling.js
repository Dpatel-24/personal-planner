...
// resolved instances (native + rolled-over — see lib/data.js's
// fetchInstancesForDateWithRollover, which returns [...rollover, ...today]
// sorted by `position`, the Board's own manual-order field, NOT a
// chronological one), stack every instance with no scheduled_start yet
// back-to-back from 8:00 AM, and leave every already-placed instance where
// it actually is.
//
...
// start+duration) so anything auto-placed after it still stacks correctly,
// but the row itself is left alone. Reloading the same day, or adding a new
// unscheduled task alongside already-scheduled ones, never reshuffles a
// time a prior pass (or a manual edit) already set.
//
// The walk that decides "leave alone vs. restack" (below) processes normal
// tasks in REAL chronological order (each one's own scheduled_start),
// re-sorted from the caller's position-ordered input — not in the
// caller's own list order. See sortKeyFor()'s and the sort call's own
// comments for why: walking in position-order instead used to silently
// undo a genuine rail drag-reorder on the next refresh whenever the
// dragged task's position value didn't already match its new time.
...
// Sort key for assignTodaySchedule's normal-task walk order (see its own
// comment for why this exists): a task's real scheduled_start-in-ms if it
// already has one on THIS day, otherwise +Infinity so unscheduled/wrong-day
// tasks always sort after every already-placed one — matching what the
// walk's cursor logic already assumes ("already-placed tasks come first,
// new ones stack after whatever's already there").
function sortKeyFor(instance, todayDateStr) {
  if (!instance.scheduled_start) return Infinity;
  const start = new Date(instance.scheduled_start);
  if (dateKeyFor(start) !== todayDateStr) return Infinity;
...
// round-trip fetch to see them. Writes only the rows that actually need a
// new value — a plain per-row update (Supabase has no bulk upsert-by-id-list
// for arbitrary per-row values without a temp table), acceptable at this
// app's scale (single user, a handful of tasks/day).
//
// pinned_position (Morning Chain/Evening Winddown) forces its own rule on
// top of the normal auto-stack below: a 'first'-pinned task ALWAYS gets
// re-clamped to the very first slot (railStartFor) and a 'last'-pinned task
// ALWAYS gets re-clamped to the very last slot (right after every other
// task), on EVERY call — not just while unscheduled, unlike every other
// task's "already scheduled, leave it alone" rule. This is what makes the
// rail (a real time-grid, not a manually-ordered list like the board) still
// honor "immune to reordering": without it, a pinned task would only ever
// land in its correct slot on its very first, never-yet-scheduled render,
// then drift wherever a drag (or just other tasks shifting around it)
// left it, exactly like a normal task.
export async function assignTodaySchedule(instances, todayDateStr, client = supabase) {
  ...
  // BUG FIX (2026-08): the single-pass cursor loop below decides "leave
  // this task alone" vs "restack it" by comparing each task's own start to
  // a `cursor` that only ever advances forward as the loop walks the
  // array — so the loop's WALK ORDER has to already match real chronological
  // order, or it self-contradicts. `instances` arrives sorted by the DB's
  // `position` column (see lib/data.js's fetchTodayAndRollover/
  // fetchInstances), which is the Board's own manual-reorder field —
  // completely unrelated to a task's actual scheduled_start on the rail.
  // Without this sort, dragging a task on the Schedule Rail to a time
  // BEFORE another task that happens to have an earlier `position` used to
  // get silently undone on the very next refresh: the loop would process
  // the position-earlier task first, advance the cursor past ITS end, then
  // reach the dragged task, see its new (earlier) start is now behind that
  // cursor, and "restack" it right back after the task it was just dragged
  // in front of — reported as "moving the third task above the second one
  // doesn't work." Sorting by each task's own valid-today scheduled_start
  // first (nulls/wrong-day pushed to the end, stable sort so their
  // relative position-order is preserved for the normal unscheduled-stack
  // case) makes the walk order match reality, so a real drag-reorder is
  // read back correctly instead of being fought on the next load.
  const normal = instances
    .filter((i) => !i.pinned_position)
    .map((instance, index) => ({ instance, index }))
    .sort((a, b) => {
      const aKey = sortKeyFor(a.instance, todayDateStr);
      const bKey = sortKeyFor(b.instance, todayDateStr);
      if (aKey !== bKey) return aKey - bKey;
      return a.index - b.index; // stable: preserve original (position) order among ties/unscheduled
    })
  ...
  // Pinned-first tasks always open the day, unconditionally re-clamped to
  // the current cursor (still just the rail floor at this point, since
  // nothing else has been placed yet).
  for (const instance of pinnedFirst) {
    const newStart = new Date(cursor).toISOString();
    if (instance.scheduled_start !== newStart) toWrite.push({ id: instance.id, scheduled_start: newStart });
  ...
  for (const instance of normal) {
    ...
    // "Leave it alone" additionally requires its own start to already be
    // AT OR AFTER the current cursor — not just "scheduled for today" in
    // isolation. Without this, a pinned-first task's forced insertion at
    // the very front (above) can open a slot that a stale already-placed
    // task from a PRIOR render is still sitting in (e.g. something last
    // auto-stacked at 8:00 AM before Morning Chain existed to claim that
    // exact slot) — that task would otherwise stay put and visually
    // collide/overlap with the pinned block now occupying the same time,
    // rather than getting bumped to right after it like everything else.
    const existingStart = instance.scheduled_start ? new Date(instance.scheduled_start) : null;
    const startsToday = existingStart && dateKeyFor(existingStart) === todayDateStr;
    if (startsToday && existingStart >= cursor) {
      ...
      const existingEnd = new Date(existingStart.getTime() + instance.estimated_duration_minutes * 60000);
  ...
  // Pinned-last tasks always close the day — unconditionally re-clamped to
  // wherever the cursor landed after every pinned-first and normal task
  // (in that order) was placed/accounted for above.
  for (const instance of pinnedLast) {
    const newStart = new Date(cursor).toISOString();
    if (instance.scheduled_start !== newStart) toWrite.push({ id: instance.id, scheduled_start: newStart });
    cursor = new Date(cursor.getTime() + instance.estimated_duration_minutes * 60000);
  }

  if (toWrite.length === 0) return instances; // fully idempotent no-op — nothing needed a new value
...
// Mirrors ScheduleRail.js's own RAIL_END_HOUR (midnight) — kept as a
// separate local constant rather than importing it, since a UI component's
// layout constant is the wrong direction of dependency for a pure lib
// function. If the rail's own grid boundary ever changes, this should be
// revisited alongside it, not silently drift. 24 is intentional, not an
// off-by-one — Date.setHours(24, 0, 0, 0) below correctly rolls over to the
// NEXT day's 00:00 as a timestamp, which is exactly the midnight boundary
// this needs; it's only ever used for a millisecond comparison, never
// displayed, so the calendar-day rollover it implies is harmless.
const CASCADE_END_OF_DAY_HOUR = 24; // midnight
...
export class CascadeEndOfDayError extends Error {
  constructor(taskId, attemptedStart) {
    super(
      `cascadeLater: pushing task ${taskId} to ${attemptedStart.toISOString()} would land past ` +
        `midnight. Not truncating or wrapping — reporting instead.`
    );
