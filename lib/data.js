...
import { sortWithPins } from './pinnedSort';
...
// The today+rollover MERGE, extracted on its own (no auto-stack side
// effect) — fetchInstancesForDateWithRollover below is this PLUS
// assignTodaySchedule; Focus Mode (components/DayAgendaRail.js /
// pages/index.js's Focus view) calls this directly instead, specifically to
// AVOID assignTodaySchedule's own scheduled_start-writing side effect —
// opening Focus mode must not silently stamp every untimed task with a
// fabricated time just because the rail's own auto-placement pass would
// normally do that. One real merge implementation either way; the two
// callers differ only in whether they also auto-place times afterward.
export async function fetchTodayAndRollover(dateStr, client = supabase) {
  ...
  return sortWithPins([...rolloverInstances, ...todayInstances]);
}
...
export async function fetchInstancesForDateWithRollover(dateStr, client = supabase) {
  const merged = await fetchTodayAndRollover(dateStr, client);
  ...
  return assignTodaySchedule(merged, dateStr, client);
}
...
// Same shape as fetchInstancesForDateWithRollover, but for a FUTURE day —
// no rollover merge (a day that hasn't happened yet has nothing to roll
// over from; pulling in TODAY's still-open rollover onto a preview of
// tomorrow would be wrong). Still auto-stacked via assignTodaySchedule so
// unscheduled items land on the rail the same way today's do. Used by
// ScheduleRail when it's pointed at a non-today dateStr (Daily Planning's
// tomorrow rail).
export async function fetchPlannedSchedule(dateStr, client = supabase) {
  const instances = sortWithPins(await fetchInstances({ from: dateStr, to: dateStr }, client));
  return assignTodaySchedule(instances, dateStr, client);
}
...
export async function overrideInstance(id, fields, client = supabase) {
  ...
  // Added for Daily Planning (Step 5 of the Focus Mode/Daily Planning
  // build): its Confirm step needs to date-roll AND reorder possibly-
  // recurring, non-override instances ("Already on Tomorrow" includes
  // fresh RRULE-generated rows) — moveInstance() is explicitly restricted
  // to one-off/already-overridden instances (see its own header comment),
  // so it can't be reused here safely. Routing every Daily Planning write
  // through overrideInstance instead (is_override:true always, same as
  // every other override path) is what makes touching a non-override
  // recurring row here safe — it's marked diverged from the template up
  // front, so the next generateInstances() pass won't regenerate/conflict
  // with it, same protection every other override write already gets.
  if ('position' in fields) patch.position = fields.position;
  const { data, error } = await client
    .from('task_instances')
    .update(patch)
    .eq('id', id)
