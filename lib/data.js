...
// Embed the parent template so we can resolve inherited title/description/tag.
const INSTANCE_SELECT =
  '*, template:task_templates(id, title, description, recurrence_rule, start_date, end_date, active, tag)';

// Resolve the effective title/description/tag for an instance. One-off tasks
// (no template) and overrides carry their own values; otherwise inherit from
// the template. Keeps `template` attached for views that need recurrence info.
...
// Distinct tag values across the WHOLE app (both tables — a template's tag
// might not yet appear on any instance if its series hasn't regenerated
// recently) — the global list for the Calendar's tag-filter dropdown, stable
// no matter which month is currently in view.
export async function fetchDistinctTags(client = supabase) {
  const [templates, instances] = await Promise.all([
    client.from('task_templates').select('tag').not('tag', 'is', null),
    client.from('task_instances').select('tag').not('tag', 'is', null),
  ]);
  if (templates.error) throw templates.error;
  if (instances.error) throw instances.error;
  const set = new Set();
  for (const row of templates.data ?? []) if (row.tag) set.add(row.tag);
  for (const row of instances.data ?? []) if (row.tag) set.add(row.tag);
...
export async function createOneOffTask(
  { title, description = '', scheduledDate, tag = null },
  client = supabase
) {
...
export async function createRecurringTask(
  { title, description = '', recurrenceRule, startDate, endDate = null, tag = null },
  client = supabase
) {
...
// Update a one-off task (no template) in place.
export async function updateOneOff(id, fields, client = supabase) {
  ...
  if ('tag' in fields) patch.tag = fields.tag;
  const { data, error } = await client
    .from('task_instances')
    .update(patch)
    .eq('id', id)
...
export async function splitTemplate(oldTemplate, fromDate, fields, client = supabase) {
  ...
  const { data: newTemplate, error: insErr } = await client
    ...
    .insert({
      ...
      tag: fields.tag ?? oldTemplate.tag,
      start_date: fromDate,
