...
// Delete a recurring task's template AND every task_instance generated from
// it — all of them, regardless of status/is_override, per the user's own
// call: deleting the series should leave nothing behind, not orphaned
// occurrences. (An earlier version of this left instances in place, relying
// on the schema's `template_id ... on delete set null` to just detach them.
// That silently broke: title/description are LIVE-joined from the template
// for any non-override instance (see resolveInstance in lib/data.js) — the
// instance's own `title` column is never actually populated while
// template_id is set, so once the template was gone, every one of those
// rows rendered as "(untitled)" instead of surviving as a real one-off
// task. Explicit delete avoids that class of bug entirely, and matches
// what "delete the recurring task" actually means to the user.)
// checklist_items and time_entries both cascade-delete from task_instances
// on their own FKs, so deleting the instances first is enough — nothing
// else needs a separate cleanup pass. Instances are deleted BEFORE the
// template row so there's no window where they'd exist. Deleted in id
// batches isn't necessary at this app's scale (single user).
export async function deleteTaskTemplate(templateId, client = supabase) {
  const { error: instErr } = await client.from('task_instances').delete().eq('template_id', templateId);
